...
from fastapi import APIRouter, Depends, HTTPException, Form, File  # <-- Added File here
from fastapi import UploadFile  # If not already
...
def create_service_endpoint(
    title: str = Form(...),  # <-- Now Form is imported
    desc: str = Form(...),
...
def delete_service_endpoint(service_id: int, db: Session = Depends(get_db), current_user=Depends(require_admin)):
    ...
    return {"message": "Deleted"}# routers/admin_router.py
from fastapi import APIRouter, Depends, HTTPException, Form  # <-- Added Form here
from sqlalchemy.orm import Session
from typing import List, Optional
from core.database import get_db
from core.security import get_current_user
