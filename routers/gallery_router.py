...
def read_gallery(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    logger.info(f"GET /gallery (skip={skip}, limit={limit})")
...
def create_image_endpoint(
    alt: str = Form(...),
    desc: str = Form(...),
    file: UploadFile = File(..., description="Imagen a subir"),  # Single UploadFile
    db: Session = Depends(get_db)
):
    logger.info(f"POST /gallery: {alt}")
...
# No-slash versions (mantén si redirect_slashes=False)
@gallery_router.get("", response_model=list[Image])
def read_gallery_no_slash(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return get_images(db, skip, limit)

@gallery_router.post("", response_model=Image, status_code=201)
def create_image_no_slash(
    alt: str = Form(...), desc: str = Form(...),
