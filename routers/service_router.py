# routers/service_router.py (actualizado)
...
from core.security import get_current_user, TokenData  # TokenData ahora tiene sub
import logging
...
def read_admin_services(
    ...
    logger.info(f"Admin {current_user.sub} fetching services")  # ← Ahora sub existe
    return get_services(db, skip=skip, limit=limit)
...
def create_admin_service(
    title: str = Form(..., description="Título del servicio"),
...
def update_admin_service(
    service_id: int,
    title: Optional[str] = Form(None, description="Nuevo título (opcional)"),
    ...
    try:
        ...
        logger.error(f"Validation error: {e}")
        raise HTTPException(status_code=404 if "no existe" in str(e) else 400, detail=str(e))
    except Exception as e:
        logger.error(f"Unexpected error: {e}")
        raise HTTPException(status_code=500, detail=f"Error: {str(e)}")
